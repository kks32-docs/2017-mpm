...
For grain-scale simulations, classical DEM approach was used. 
...
cell was investigated by~\citep{soundararajan2015}. The initial 
configuration of the slope in MPM is shown in~\cref{fig:mpm_slope_setup}. 
Frictional boundary conditions are applied on the left and the bottom 
boundaries by applying constraints to the nodal acceleration. Mohr-Coulomb
model with no dilation is used to simulate the continuum behaviour of the
granular pile. Periodic shear tests in DEM reveals a macroscopic friction 
coefficient of 0.22.
